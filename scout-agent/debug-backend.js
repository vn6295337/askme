...
async function generateValidationReport() {
  ...
  const report = `# AskMe CLI Model Validation Report
...
${validatedModels.map(model => `- **${model.provider}**: ${model.model_name} (${model.health_status || 'unknown status'})`).join('\n')}

### Provider Performance

${connectivityResults.filter(r => r.test.includes('Provider') && r.status === 'success').map(result => {
  return `- **${result.test.replace(' Provider', '')}**: ${result.response_time}ms response time`;
}).join('\n')}

## 🚫 Excluded Models

${excludedModels.slice(0, 10).map(model => `- **${model.provider}**: ${model.model_name} - ${model.reason}`).join('\n')}

${excludedModels.length > 10 ? `\n*... and ${excludedModels.length - 10} more excluded models*` : ''}

## 🏆 Top Eligible Models

${eligibleModels.slice(0, 10).map((model, index) => {
  return `${index + 1}. **${model.provider}/${model.model_name}** (Score: ${model.eligibility_score})`;
}).join('\n')}

## 🛠️ Recommendations

### High Priority
${connectivityResults.filter(r => r.status === 'error').length > 0 ? '- ⚠️ Fix failing backend connectivity tests' : '- ✅ All backend tests passing'}
...
- **Average Response Time:** ${connectivityResults.filter(r => r.status === 'success' && r.response_time).reduce((sum, r) => sum + r.response_time, 0) / connectivityResults.filter(r => r.status === 'success' && r.response_time).length || 0}ms
- **Success Rate:** ${((connectivityResults.filter(r => r.status === 'success').length / connectivityResults.length) * 100).toFixed(1)}%
- **Model Coverage:** ${validatedModels.length} models across ${[...new Set(validatedModels.map(m => m.provider))].length} providers
