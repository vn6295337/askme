...
object IntelligentProviderManager {
    ...
    suspend fun getSmartResponse(prompt: String): String {
        ...
        for (providerName in rankedProviders.map { it.name }) {
            ...
            val result = tryProviderWithMetrics(providerName, prompt)
        ...
        return "❌ All providers failed. Please check your internet connection and try again."
    }
    
    private suspend fun tryProviderWithMetrics(providerName: String, prompt: String): String {
        val startTime = System.currentTimeMillis()
        
        return try {
            val result = withTimeoutOrNull(30000) { // 30 second timeout
                when (providerName) {
                    "google" -> GoogleGemini.chat(prompt)
                    "mistral" -> MistralAI.chat(prompt)
                    "llama" -> LlamaAI.chat(prompt)
