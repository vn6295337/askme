...
/**
 * Security Test: Unauthorized File Access Prevention for CLI Application
 * Tests that the application properly restricts file access outside allowed directories
 */
class CliUnauthorizedFileAccessTest {
    ...
    // Simulate file access attempt with security validation - SIMPLIFIED LOGIC
    private fun attemptFileAccess(filePath: String): FileAccessResult {
        // Simple string-based checks instead of complex regex
        val allowedPrefixes = listOf("config/", ".askme/")
        val allowedExact = listOf("settings.json", "config.json", "api-keys.enc")
        
        // Check for obvious directory traversal
        val hasDirTraversal = filePath.contains("../") || 
                            filePath.contains("..\\") || 
                            filePath.contains("%2e%2e") ||
                            filePath.contains("%252f")
        
        // Check for system paths
        val isSystemPath = filePath.startsWith("/etc/") ||
        ...
        // If it has traversal or is system path, deny
        if (hasDirTraversal || isSystemPath) {
        ...
        val isAllowed = allowedPrefixes.any { filePath.startsWith(it) } ||
                       allowedExact.any { filePath == it || filePath.endsWith("/$it") }
