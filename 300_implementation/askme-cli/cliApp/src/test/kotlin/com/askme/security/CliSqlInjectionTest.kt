...
/**
 * Security Test: SQL Injection Prevention for CLI Application
 * Tests input sanitization and validation to prevent SQL injection attacks
 */
class CliSqlInjectionTest {
...
// Mock input validation function with comprehensive threat detection
    private fun validateInput(input: String): ValidationResult {
        // Comprehensive SQL injection detection patterns
        val sqlThreats = listOf(
            "DROP TABLE", "DELETE FROM", "TRUNCATE", "ALTER TABLE",
            "' OR '1'='1", "' OR 1=1", "' AND '1'='1", "' AND 1=1",
            "UNION SELECT", "UNION ALL SELECT",
            "EXEC", "EXECUTE", "xp_cmdshell", "sp_executesql",
            "WAITFOR DELAY", "SELECT SLEEP", "BENCHMARK",
            "information_schema", "sysobjects", "mysql.", "pg_",
            "INSERT INTO", "UPDATE SET",
            "COUNT(*) FROM", "LENGTH(password)", "ASCII(SUBSTRING",
            "IF (1=1)", "db.users.drop", "db.users.find",
            "return true", "this.password"
        )
        ...
        sqlThreats.forEach { threat ->
            if (inputUpper.contains(threat.uppercase())) {
                detectedThreats.add("SQL injection pattern: $threat")
            }
        }
        
        // Check for URL encoded SQL injection
        val encodedPatterns = listOf(
            "%27%3B", "%27%20OR", "%20UNION%20", "%2D%2D",
            "%27%29%3B", "%20DELETE%20"
        )
        
        encodedPatterns.forEach { pattern ->
            if (input.contains(pattern)) {
                detectedThreats.add("URL-encoded SQL injection")
            }
        }
        
        // Check for comment patterns
        if (input.contains("--") || input.contains("/*") || input.contains("*/")) {
            detectedThreats.add("SQL comment injection")
        }
        ...
        return if (detectedThreats.isNotEmpty()) {
            ...
            ValidationResult(
                isValid = sanitized.isNotEmpty() && sanitized.length <= 1000,
